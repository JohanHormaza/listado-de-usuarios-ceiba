...
+ (void)raiseEventFromDifferentThreadWithExplicitSender:(id)originalObject :(id)sender :(NSString *)suffix :(NSArray *)params;
...
@interface B4IArray : NSObject<B4IIterable, B4IDebuggable> {
    @public NSData* internalBytesData;
...
@property (nonatomic) NSMutableDictionary *waitForEvents;
+ (B4I *)shared;
...
+ (id)runDynamicMethod:(id)target method:(NSString *)e throwErrorIfMissing:(BOOL)throwError args:(NSArray *)p;
...
- (BOOL)checkAndRunWaitForEvent:(id)sender event:(NSString *)e params:(NSArray *)p;
@end
//~hide
@interface B4IResumableSub : NSObject {
@public int _state;
@public int _catchState;
}
@property (nonatomic) int state;
@property (nonatomic) int catchState;
@property (nonatomic, weak) B4I* bi;
@property (nonatomic) B4I* waitForB4I;
@property (nonatomic) BOOL completed;
- (void)resume:(B4I*) bi :(NSArray *)result;
@end
//~hide
@interface B4IWaitForEvent : NSObject
@property (nonatomic) B4IResumableSub *rs;
@property (nonatomic, weak) NSObject *senderFilter;
@property (nonatomic) BOOL senderFilterSet;
@property (nonatomic, readonly) BOOL noFilter;
@property (nonatomic, readonly) BOOL cleared;

- (instancetype) init:(B4IResumableSub *)rs1 :(NSObject *)senderFilter1;
@end
//~hide
...
//~hide
@property (nonatomic) BOOL debugFlushLogs;
...
/**
 * Loads a bitmap and resizes it based on the specified Width and Height.
 *KeepAspectRatio - Whether to preserve the image aspect ratio.
 */
- (B4IBitmap *)LoadBitmapResize:(NSString *)Dir :(NSString *)FileName :(float)Width :(float)Height :(BOOL)KeepAspectRatio;
...
//~hide
- (void)LogImpl:(NSString *)line :(NSString *)Message :(int)Color;
...
*/
//~RaisesSynchronousEvents:true
- (NSObject*)Msgbox2:(B4I *)bi :(NSString *)EventName :(NSString *)Message :(NSString *)Title :(B4IList *)Buttons;
//~hide
...
- (void)Sleep:(B4I *)bi :(B4IResumableSub *)rs :(int)Milliseconds;
/**
...
//~hide
- (void)WaitFor:(NSString *)SubName :(B4I *)bi :(B4IResumableSub *)rs :(NSObject *)SenderFilter;
 ...
 */
- (void)SetDebugAutoFlushLogs:(BOOL)Enabled;
 ...
 */
- (NSObject *)IIf:(BOOL)Condition :(NSObject *)TrueValue :(NSObject *)FalseValue;

//~hide
- (void)ReturnFromResumableSub:(B4IResumableSub *)rs :(NSObject *)returnValue;
@end
//~hide
@interface B4IStaticModule:B4IObjectWrapper {
@public B4ICommon *___c;
}
 ...
 *You can use it as the sender filter parameter and for the Complete event.
 */
//~Shortname: ResumableSub
//~ObjectWrapper: B4IResumableSub*
@interface B4IResumableSubWrapper : B4IObjectWrapper
...
//~Event:JSComplete (Success As Boolean, Result As String)
...
- (void)Initialize:(B4I *)bi :(NSString *)EventName;
//Initializes the WebView with a custom configuration (created with NativeObject).
- (void)InitializeWithCustomConfiguration:(B4I *)bi :(NSString *)EventName :(WKWebViewConfiguration *)Configuration;
...
- (void)Release;
/**
 *Asynchronously evaluates the script. Returns an object that should be used as the sender filter parameter.
 *Example:<code>
 *Dim sf As Object = WebView1.EvaluateJavaScript($"document.body.style.backgroundColor = '#DDC8B0'"$)
 *Wait For (sf) WebView1_JSComplete (Success As Boolean, Result As String)</code>
 */
- (NSObject *)EvaluateJavaScript:(NSString *)Script;

/**
* Loads a url. Note that you cannot use this method to load files from the Assets folder.
* Example:<code>
* WebView1.LoadUrl("https://www.google.com")</code>
*/
...
//Gets or sets the spell checking behavior (one of the SPELLCHECK constants);
@property (nonatomic)int SpellChecking;
...
@property (nonatomic) B4IAttributedString *AttributedText;
...
+ (B4IFontWrapper *)fromDesignerMap:(NSMutableDictionary *)props :(float)scale :(NSMutableDictionary *)parentProps;
//Creates a new FontAwesome font.
- (B4IFontWrapper *)CreateFontAwesome:(float)Size1;
//Creates a new Material Icons font.
- (B4IFontWrapper *)CreateMaterialIcons:(float)Size1;
...
@property (nonatomic) B4IViewWrapper *TitleView;
...
/**
*Returns the distances from each of the borders that are considered safe to use for the user interface.
*This is relevant for devices such as iPhone X where parts of the screen are hidden.
*The object itself does not represent a rectangle. It is a set of four distances.
*Usage example:
<code>
*Sub Page1_Resize (Width As Float, Height As Float)
*	Dim r As Rect = Page1.SafeAreaInsets
*	root.SetLayoutAnimated(0, 1, r.Left, r.Top, Width - r.Right - r.Left, Height - r.Bottom - r.Top)
*End Sub</code>
*/
@property (nonatomic, readonly)B4IRect* SafeAreaInsets;
- (void)Initialize:(B4I *)bi :(NSString *)EventName;
...
//~inline:))
//Returns the bitwise AND of the two values.
- (long long)AndLong:(long long)n1 :(long long)n2;
...
//~inline:))
//Returns the bitwise OR of the two values.
- (long long)OrLong:(long long)n1 :(long long)n2;
...
//~inline:))
//Returns the bitwise XOR of the two values.
- (long long)XorLong:(long long)n1 :(long long)n2;
...
* Shift - Number of positions to shift.
*/
- (long long)ShiftLeftLong:(long long)n :(int)shift;
//~inline:((
//~inline:) >> (
//~inline:))
/**
...
* Shift - Number of positions to shift.
*/
- (long long)ShiftRightLong:(long long)n :(int)shift;
...
* Shift - Number of positions to shift.
*/
- (long long)UnsignedShiftRightLong:(long long)n :(int)shift;
...
* Radix - Should be between 2 to 36.
*/
- (long long)ParseLong:(NSString *)value :(int)radix;
...
/**
 * Gets a byte from an array of bytes.
 * The performance improvement over the standard method is only relevant when making millions of calls.
 */
//~inline:(((unsigned char*)((
//~inline:)->internalBuffer))[
//~inline:])
- (unsigned char)FastArrayGetByte:(B4IArray */*unsigned char,1*/)Array :(int)Index;
/**
 * Sets a byte in an array of bytes.
 * The performance improvement over the standard method is only relevant when making millions of calls.
 */
//~inline:((unsigned char*)((
//~inline:)->internalBuffer))[
//~inline:] = (
//~inline:)
- (void)FastArraySetByte:(B4IArray */*unsigned char,1*/)Array :(int)Index :(unsigned char)Value;
/**
 * Gets an int from an array of bytes. The size of an int is 4 bytes.
 * The performance improvement over the standard method is only relevant when making millions of calls.
 */
//~inline:(((int*)((
//~inline:)->internalBuffer))[
//~inline:])
- (int)FastArrayGetInt:(B4IArray */*unsigned char,1*/)Array :(int)Index;
/**
 * Sets an int in an array of bytes. The size of an int is 4 bytes.
 * The performance improvement over the standard method is only relevant when making millions of calls.
 */
//~inline:((int*)((
//~inline:)->internalBuffer))[
//~inline:] = (
//~inline:)
- (void)FastArraySetInt:(B4IArray */*unsigned char,1*/)Array :(int)Index :(int)Value;
/**
 * Gets a float from an array of bytes. The size of a float is 4 bytes.
 * The performance improvement over the standard method is only relevant when making millions of calls.
 */
//~inline:(((float*)((
//~inline:)->internalBuffer))[
//~inline:])
- (float)FastArrayGetFloat:(B4IArray */*unsigned char,1*/)Array :(int)Index;
/**
 * Sets a float in an array of bytes. The size of a float is 4 bytes.
 * The performance improvement over the standard method is only relevant when making millions of calls.
 */
//~inline:((float*)((
//~inline:)->internalBuffer))[
//~inline:] = (
//~inline:)
- (void)FastArraySetFloat:(B4IArray */*unsigned char,1*/)Array :(int)Index :(float)Value;
#define B4I_MEMCPY(Src, Dest, Count) memcpy(Dest, Src, Count)
//~inline:B4I_MEMCPY((
//~inline:)->internalBuffer + (
//~inline:), (
//~inline:)->internalBuffer + (
...
*/
- (void)ArrayCopy:(B4IArray */*byte,1*/)Src :(int)SrcOffset :(B4IArray */*byte,1*/)Dest :(int)DestOffset :(int)Count;
@end
//~hide
 ...
 */
- (NSObject *)Copy2Async:(B4I *)bi :(B4IInputStream *)In :(B4IOutputStream *)Out;
/**
 * Asynchronous version of ListFiles. Should be used with Wait For.
 * Example: <code>Wait For (File.ListFilesAsync(Dir)) Complete (Success As Boolean, Files As List)</code>
 */
- (NSObject *)ListFilesAsync:(B4I *)bi :(NSString *)Dir;
...
*Log("Success: " & Success)</code>
*/
- (NSObject *)CopyAsync:(B4I *)bi :(NSString *)DirSource :(NSString *)FileSource :(NSString *)DirTarget :(NSString *)FileTarget;
/**
...
*/
- (void)WriteBytes:(NSString *)Dir :(NSString *)FileName :(B4IArray */*byte,1*/)Data;
...
//Returns the bitmap scale. Note that unlike in B4A, the Width and Height are always normalized (as if the scale is 1).
...
//Loads an image and resizes it based on the width and height.
- (void)InitializeResize:(NSString *)Dir :(NSString *)FileName :(float)Width :(float)Height :(BOOL)KeepAspectRatio;
/**
 * Resizes the current bitmap and returns a <b>new</b> bitmap.
 *Width - New bitmap width.
 *Height - New bitmap height.
 *KeepAspectRatio - Whether to preserve the aspect ratio.
 */
- (B4IBitmap *)Resize:(float)Width :(float)Height :(BOOL)KeepAspectRatio;

/**
 * Returns a <b>new</b> rotated bitmap. The bitmap is rotated clockwise.
 */
- (B4IBitmap *)Rotate:(float)Degrees;
//Returns a <b>new</b> cropped bitmap.
- (B4IBitmap *)Crop:(float)Left :(float)Top :(float)Width :(float)Height;
...
/**
 * Similar to DrawPath. Draws a rotated path.
 */
- (void)DrawPathRotated:(UIBezierPath *)Path :(int)Color :(BOOL)Filled :(float)StrokeWidth :(float)Degrees :(float)CenterX :(float)CenterY;
...
//~hide
- (void)DrawText2:(NSString *)Text :(float)x :(float)y :(B4IFontWrapper *)Font :(int)Color :(NSString *)Align;
//~hide
- (void)DrawTextRotated2:(NSString *)Text :(float)x :(float)y :(B4IFontWrapper *)Font :(int)Color :(NSString *)Align :(float)Degrees;
...
//Returns 1 as the scale is normalized automatically by the OS.
@property (nonatomic, readonly) float Scale;
...
//~ObjectWrapper: NSAttributedString*
/**
 * CSBuilder is similar to StringBuilder, however it creates AttributedStrings.
 *These are strings that include styling information.
 *All styling methods begin a span. The span will end (closed) when you call Pop or PopAll.
 *It is convenient to call PopAll at the end to close any open spans.
 *Note that changing the font should be done in the parent resize event when AutoScaleAll is enabled.
 *Example:<code>
 * Dim cs As CSBuilder
 * Label1.AttributedText = cs.Initialize.Colors(Colors.Red).Underline.Append("Hello ").Pop.Append("World!!!").PopAll
 * </code>
 */
@interface B4ICSBuilder : B4IObjectWrapper
@property (nonatomic, readonly) int Length;
- (B4ICSBuilder *)Initialize;
/**
 * Appends text.
 */
- (B4ICSBuilder *)Append:(NSString *)Text;
//Sets the vertical offset (positive = downwards).
- (B4ICSBuilder *)VerticalAlign:(int)Shift;
//Sets the text alignment. One of the following values: ALIGN_LEFT, ALIGN_CENTER or ALIGN_RIGHT.
- (B4ICSBuilder *)Alignment:(NSString *)Alignment;
/**
 * Creates a link. Links will be clickable in non-editable TextViews.
 */
- (B4ICSBuilder *)Link:(NSString *)URL;
//Starts an underline span.
- (B4ICSBuilder *)Underline;
//Starts a strikethrough span.
- (B4ICSBuilder *)Strikethrough;
//Starts a text color span.
- (B4ICSBuilder *)Color:(int)Color;
//Starts a background color span.
- (B4ICSBuilder *)BackgroundColor:(int)Color;
/**
 * Starts a font span. Note that when AutoScaleAll is called the font is reset.
 *You should change the font in the parent Resize event or remove the call to AutoScaleAll from the layout
 *designer script.
 */
- (B4ICSBuilder *)Font:(B4IFontWrapper *)Font;
//Sets the kerning (horizontal spacing) scale.
- (B4ICSBuilder *)KerningScale:(float)Scale;
//Closes the last added span.
- (B4ICSBuilder *)Pop;
